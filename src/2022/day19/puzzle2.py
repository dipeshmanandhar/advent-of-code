...
def max_geodes(bp: Blueprint, ore=0, clay=0, obsidian=0, geode=0, ore_bots=1, clay_bots=0, obsidian_bots=0, geode_bots=0, minutes_left=32):
    if minutes_left <= 0:
        return geode, [], [], [], []
...
def quality(bp: Blueprint):
    ...
    print(f"[{','.join([f'{i+1:2}' if i>0 else f'{i+1}' for i in range(32)])}]")
    print(ore_bots)
    print(clay_bots)
    print(obsidian_bots)
    print(geode_bots)
