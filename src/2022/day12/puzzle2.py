...
def bfs(grid, graph, start_row, start_col):
    num_rows = len(graph)
    ...
    prevs = [[None] * num_cols for _ in range(num_rows)]
    ...
    while unvisited:
        ...
        for next_r, next_c in graph[r][c]:
            if not visited[next_r][next_c]:
                ...
                prevs[next_r][next_c] = (r, c)
                if grid[next_r][next_c] == 0:
                    return shortest_dist[next_r][next_c]
    return float('inf')
...
print(bfs(grid, graph, end_row, end_col))
