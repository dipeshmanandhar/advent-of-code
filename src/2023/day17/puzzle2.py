...
class Direction(Enum):
    ...
    def possible_next_directions(self, count: int) -> list[tuple[Direction, int]]:
        next_directions = [(dir, MIN_BLOCKS) for dir in Direction.all()]
        next_directions.remove((self.reverse(), MIN_BLOCKS))
        if count >= MAX_BLOCKS:
            next_directions.remove((self, MIN_BLOCKS))
        else:
            next_directions[next_directions.index((self, MIN_BLOCKS))] = (
                self,
                count + 1,
            )
...
def min_heat_loss(heat_loss_map: list[list[int]]):
    ...
    while curr_nodes:
        ...
        for next_direction, next_direction_count in next_directions:
            next_r, next_c = r, c
            steps = next_direction_count
            if next_direction == curr_direction:
                steps -= curr_direction_count
            mid_points = []
            if next_direction == Direction.UP:
                next_r -= steps
                if steps > 1:
                    mid_points = [(mid_r, c) for mid_r in range(r - 1, next_r, -1)]
            elif next_direction == Direction.LEFT:
                next_c -= steps
                if steps > 1:
                    mid_points = [(r, mid_c) for mid_c in range(c - 1, next_c, -1)]
            elif next_direction == Direction.DOWN:
                next_r += steps
                if steps > 1:
                    mid_points = [(mid_r, c) for mid_r in range(r + 1, next_r)]
            else:
                next_c += steps
                if steps > 1:
                    mid_points = [(r, mid_c) for mid_c in range(c + 1, next_c)]
            if next_r >= 0 and next_r < NUM_ROWS and next_c >= 0 and next_c < NUM_COLS:
                ...
                for mid_r, mid_c in mid_points:
                    next_total_heat_loss += heat_loss_map[mid_r][mid_c]
